...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [mypluginPlugin],
    });

    await request(server).get('/api/myplugin/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/myplugin/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/myplugin/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/myplugin/todos/${createdTodoItem.id}`)
